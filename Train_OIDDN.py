...
log_file_name = "./%s/Log_OIDN_layer_%d_ratio_%d.txt" % (args.log_dir, layer_num, cs_ratio)
...
for epoch_i in range(start_epoch+1, end_epoch+1):
    for data in rand_loader:
        ...
        for k in range(len(phis)):            
            loss_orth += torch.mean(torch.pow(torch.mm(phis[k], torch.transpose(phis[k], 0, 1))- Eye_I, 2))                              

        # Zero gradients, perform a backward pass, and update the weights.
        optimizer.zero_grad()
        torch.autograd.backward((loss_discrepancy,torch.mul(gamma,loss_symmetry),torch.mul(mu,loss_orth)))
        optimizer.step()
